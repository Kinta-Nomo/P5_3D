...
function draw(){
    ...
    for (var i in original){
      ...
      var furthest = distance(rotationOrigin, original[i][0])
      var deepest = original[i][0][2]
      ...
      for (var j = 0; j < original[i].length-1; j++){
        ...
        if (furthest < pointDistance){
          furthest = pointDistance
        }
        if (deepest < original[i][j][2]){
          deepest = original[i][j][2]
        }
        
      }
      var newFace = original[i]
      averagess.push([average[0]/(original[i].length-1), average[1]/(original[i].length-1), average[2]/(original[i].length-1)])
      average = distance(rotationOrigin, [average[0]/(original[i].length-1), average[1]/(original[i].length-1), average[2]/(original[i].length-1)])
      ...
      //descending arranged with nearestpoint
      for (var j = 0; j  < arranged.length; j++){
        if (arranged[j][1] <= deepest + average){
          arranged.splice(j, 0, [newFace, deepest + average])
          break
        }else if(j == arranged.length-1){
          arranged.push([newFace,  deepest + average])
          break
        }
      }
    }
      ...
      for (var j = 0; j < arranged[i][0].length-1; j++){
        ...
        // if (bearing < PI/4 && bearing > -PI/4){
          ...
          if (arranged[i][0][j][2] > rotationOrigin[2]+screenDepth){
            ...
            stroke(255)
            point(-arranged[i][0][j][2],-arranged[i][0][j][1])
            stroke(255, 0, 0)
          }else{
